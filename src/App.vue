...
export default {
 ...
 methods: {
   filteredPokemons(filterName) {
     if ( !filterName ) {
       this.pokemonsFilters = [...this.pokemons]
       return
     } 
     // console.log(filterName, 'desde filter function');
     filterName = filterName.toLowerCase();

     let pokemonsWithFilter = this.pokemons.filter( pokemon => pokemon.name.toLowerCase().includes(filterName))

     this.pokemonsFilters = [...pokemonsWithFilter]

   },
   handleUpdateSearchTerm(filterName) {
     // console.log(algo);
     // console.log('padre recibiendo el emit')
     this.filteredPokemons(filterName);
   },
